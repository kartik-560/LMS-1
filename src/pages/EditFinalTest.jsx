    ...
    const transformQuestionsFromAPI = (apiQuestions) => {
        ...
        return apiQuestions.map((q) => {
            const baseQuestion = {
                ...
                executionTimeLimit: q.executionTimeLimit || 3000,
        ...
        for (let i = 0; i < lesson.questions.length; i++) {
            ...
            if (q.type === "coding") {
                if (!q.testCases || q.testCases.length === 0) {
                    setError(`Question ${i + 1}: Please add at least one test case.`);
                    return false;
                }
                const hasEmptyOutput = q.testCases.some(tc => !tc.expectedOutput.trim());
                if (hasEmptyOutput) {
                    setError(`Question ${i + 1}: All test cases must have an expected output.`);
                    return false;
                }
            }
