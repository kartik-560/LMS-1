...
export default function SuperAdminDashboardPage() {
  ...
  const getFilterOptions = useCallback(() => {
  if (studentFilter === 'college') {
    // Get unique college names from students
    return [...new Set(allStudents.map(s => s.collegeName).filter(Boolean))].sort();
  }
  if (studentFilter === 'department') {
    // Get unique department names from students
    return [...new Set(allStudents.map(s => s.departmentName).filter(Boolean))].sort();
  }
  return [];
 ...
 const getFilteredStudents = useCallback(() => {
  ...
  if (studentFilter !== 'all' && selectedFilterValue) {
    filtered = filtered.filter(student => {
      if (studentFilter === 'college') {
        return student.collegeName === selectedFilterValue;
      }
      if (studentFilter === 'department') {
        return student.departmentName === selectedFilterValue;
      }
      return true;
    });
  }
    ...
    <div className="space-y-6">
      <Card className="p-5 sm:p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Badge variant="info">{allCourses.length} Total Courses</Badge>
        </div>

        {allCourses.length === 0 ? (
          <div className="text-center py-12">
            <BookOpen size={48} className="mx-auto text-gray-400 mb-4 opacity-50" />
            <p className="text-gray-600 font-medium">No courses available</p>
            <p className="text-gray-500 text-sm mt-1">Create a course to get started</p>
          </div>
        ) : (
          <div className="space-y-4">
                ...
                >
                  <div className="flex flex-col md:flex-row md:items-start justify-between gap-3 mb-3">
                    ...
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      <div className="w-16 h-12 rounded-lg overflow-hidden bg-gray-100 flex-none">
                        <img
                      ...
                      <div className="min-w-0 flex-1">
                        ...
                        <div className="flex items-center gap-3 mt-1">
                          ...
                          <span className="text-xs text-gray-500">
                            {instructors.length} Instructor{instructors.length !== 1 ? 's' : ''} • {students.length} Student{students.length !== 1 ? 's' : ''}
                          </span>
                        </div>
                      </div>
                    </div>
                    ...
                    <div className="flex gap-2 flex-wrap items-center">
                      ...
                      <Link
                        ...
                        className="bg-blue-500 text-white hover:bg-blue-600 px-3 py-2 rounded text-sm inline-flex items-center"
                      >
...
<TabsContent value="students">
  <Card className="p-4 sm:p-5">
    ...
    <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 mb-4">
      ...
      {studentFilter !== 'all' && (
        <select
          ...
          className="px-3 py-2 text-sm border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 focus:border-primary-500"
        >
          <option value="">Select {studentFilter === 'college' ? 'College' : 'Department'}</option>
    ...
    <Badge variant="info" className="mb-4">
      {getFilteredStudents().length} of {allStudents.length} students
    </Badge>
      ...
      <table className="min-w-full divide-y divide-gray-200">
        <thead className="bg-gray-50">
          <tr>
            ...
            <th className="px-4 py-3 text-center text-xs font-medium text-gray-500 uppercase">Enrolled Courses</th>
              ...
              <tr key={student.id} className="hover:bg-gray-50 transition">
                <td className="px-4 py-4">
                  <div className="flex items-center gap-3">
                    ...
                    <div className="min-w-0">
                      <h4 className="font-medium text-gray-900 truncate">{student.name || 'Unnamed'}</h4>
