...
export default function SuperAdminDashboardPage() {
  ...
  const fetchCollegePermissions = async () => {
    ...
    if (!collegeId) return;
    if (isFetchingPermissions.current) return; // Prevent double-fetch
  ...
  const collegeId = selectedCollegeId || collegeDetailVM?.college?.id;
  useEffect(() => {
    // Fetch only on permissions tab, and when id is ready
                            ...
                            <div className="space-y-6">
                              ...
                              <Card className="p-5">
                                ...
                                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                                  ...
                                  <div>
                                    <label className="block text-xs text-gray-600 mb-1">
                                      Max Departments Allowed
                                    </label>
                                    <Input
                                      type="number"
                                      min={0}
                                      value={permLimits.departmentLimit || 0}
                                      onChange={(e) =>
                                        setPermLimits((s) => ({
                                          ...s,
                                          departmentLimit: Number(e.target.value) || 0,
                                        }))
                                      }
                                    />
                                  </div>
                      ...
                      {filteredColleges.length === 0 ? (
                        ...
                        filteredColleges.map((college) => {
                          ...
                          return (
                            <tr
                              key={college.id}
                              className={`border-b border-gray-100 hover:bg-gray-50 transition cursor-pointer ${selectedCollegeId === college.id ? 'bg-primary-50' : ''
                                }`}
